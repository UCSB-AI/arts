...
def main():
    ...
    for ax, task in zip(axes.flatten(), TASKS_HARD):
        plot_task(ax, task)
    out_pdf = OUT_DIR / 'progression_all.pdf'
    out_png = OUT_DIR / 'progression_all.png'
    ...
    for ax, task in zip(axes, TASKS_WINS):
        plot_task(ax, task)
    out_pdf = OUT_DIR / 'progression_wins.pdf'
    out_png = OUT_DIR / 'progression_wins.png'
    fig.savefig(out_pdf, bbox_inches='tight')
    fig.savefig(out_png, dpi=220, bbox_inches='tight')
    print(f'wrote {out_pdf}')
    plt.close(fig)
    ...
    for task in TASKS:
        ...
        p1 = OUT_DIR / f'progression_{fname}.pdf'
        p2 = OUT_DIR / f'progression_{fname}.png'
